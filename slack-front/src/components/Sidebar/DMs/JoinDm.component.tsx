...
//   }, 300);
  ...
  return (
    <Modal close={props.exitCallback} title="Direct Messages">
        ...
        <Query query={allUsersQuery} variables={{ currentUserId: user, filter: "%" }}>
        {({ data, loading,refetch }: QueryResult) => {
          ...
          return (
              <DataContainer>
                {data.User.map((user: { id: string; username: string ,color:string}) => (
                  <DataItem key={user.id} onClick={() => setSelectedUser((prevState: User[]) => {   
                    if(prevState.find(us => us.id === user.id)){
                        return prevState;
                    }
                   return [...prevState,{...user, color: colors[random(0,4)]}] as User[];})}>@ {user.username}</DataItem>
                ))}
              </DataContainer>
 ...
 export const JoinDmComponent = withApollo<{exitCallback: () => void,client:any},{}>(JoinDM);